...
const findAllRecords = async (Model) => {
  try {
    ...
    if (!records) {
      throw ApiError.notFound(messages.errors.actionFailed("find", "records"));
    }
...
const findRecordByField = async (field, value, Model) => {
  try {
    ...
    if (!record) {
      throw ApiError.notFound(messages.errors.actionFailed("find", "record"));
    }
...
const findRecordByFieldInclude = async (
  Model,
  wheres,
  fields = [],
  includes
  ...
  try {
    ...
    if (!wheres) {
      throw ApiError.badRequest(messages.errors.nullData("Filter", "Value"));
    }

    if (!fields) {
      throw ApiError.badRequest(messages.errors.nullData("Fields", "Value"));
    }

    if (!includes) {
      throw ApiError.badRequest(messages.errors.nullData("Includes", "Value"));
    }
